...
describe('PromotionType Enum', function (): void {
    describe('label method', function (): void {
        it('returns correct label for Percentage', function (): void {
            expect(PromotionType::Percentage->label())->toBe('Percentage Discount');
        });

        it('returns correct label for Fixed', function (): void {
            expect(PromotionType::Fixed->label())->toBe('Fixed Amount Discount');
        });

        it('returns correct label for BuyXGetY', function (): void {
            expect(PromotionType::BuyXGetY->label())->toBe('Buy X Get Y Free');
        });
    });
    ...
    describe('color method', function (): void {
        ...
        it('returns correct color for Fixed', function (): void {
            expect(PromotionType::Fixed->color())->toBe('warning');
        });

        it('returns correct color for BuyXGetY', function (): void {
            expect(PromotionType::BuyXGetY->color())->toBe('info');
        });
    });
    ...
    describe('formatValue method', function (): void {
        ...
        it('formats fixed value correctly', function (): void {
            expect(PromotionType::Fixed->formatValue(1000))->toBe('RM 10.00');
            expect(PromotionType::Fixed->formatValue(2500))->toBe('RM 25.00');
            expect(PromotionType::Fixed->formatValue(99))->toBe('RM 0.99');
        });

        it('formats BuyXGetY value', function (): void {
            expect(PromotionType::BuyXGetY->formatValue(1))->toBe('Buy & Get');
            expect(PromotionType::BuyXGetY->formatValue(100))->toBe('Buy & Get');
        });
    });
